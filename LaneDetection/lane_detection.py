...
def process_frame(frame):
    ...
    fps = round(1.0 / (time.time() - start_time + 0.000001), 1)    # Here we calculate the fps
...
def main():
    print("ok")
    video = cv2.VideoCapture("white_road.mp4")
    while True:
        ret, frame = video.read()

        if ret:
            end_result = process_frame(frame)
            cv2.imshow("Result", end_result) # Output processed frames
